...
assert.match(source, /starCount = Number\(user\.stars \|\| 0\)\.toLocaleString\('vi-VN'\)/, 'The admin header must display a Vietnamese-formatted star balance.');
...
assert.match(migration, /GREATEST\(COALESCE\(stars, 0\), 1000\)/, 'The admin demo migration must guarantee at least 1,000 stars.');
