const { randomInt, shuffle, formatNumber } = require('./shared');
...
function readNumber(value) {
    if (!Number.isInteger(value) || value < 0 || value >= 1_000_000_000) throw new Error('Number must be in 0..999,999,999.');
...
function parseShapes(raw) {
    ...
    if (!Array.isArray(shapes) || !shapes.length) throw new Error('At least one matching shape is required.');
    return shapes.map(item => {
        const match = String(item).match(/^(\d+):(\d+)$/);
        if (!match) throw new Error('Shapes must use LEFT:RIGHT, for example 5:4.');
        const shape = [Number(match[1]), Number(match[2])];
        if (Math.min(...shape) < 1 || Math.abs(shape[0] - shape[1]) !== 1) throw new Error('Each matching shape must differ by one item.');
        return shape;
    });
}
...
function pickDigit(digits, weights, random) {
    ...
    if (total <= 0) throw new Error('Digit weights must be positive.');
...
function generateMatchNumberWords(config = {}, random = Math.random) {
    ...
    if (!digits.length || digits.some(digit => !Number.isInteger(digit) || digit < 1 || digit > 9) || !['balanced', 'random', 'cycle'].includes(strategy)) {
        throw new Error('Invalid number-word matching configuration.');
    }
    ...
    for (let attempt = 0; attempt < 1000; attempt++) {
        ...
        while (numbers.size < total) {
            const digit = digitPlan[numbers.size];
            numbers.add(randomInt(10 ** (digit - 1), 10 ** digit - 1, random));
        }
        values = [...numbers];
        if (!config.prefixWords || new Set(values.slice(0, matchCount).map(value => readNumber(value).toLowerCase().split(' ').slice(0, config.prefixWords).join(' '))).size === 1) break;
    }
    if (!values || (config.prefixWords && new Set(values.slice(0, matchCount).map(value => readNumber(value).toLowerCase().split(' ').slice(0, config.prefixWords).join(' '))).size !== 1)) throw new Error('Không thể tạo câu với số từ tiền tố chung đã chọn.');
    ...
    const answers = shuffle(matched, random).map(value => `${formatNumber(value)}:${readNumber(value)}`);
    return {
...
module.exports = generateMatchNumberWords;
