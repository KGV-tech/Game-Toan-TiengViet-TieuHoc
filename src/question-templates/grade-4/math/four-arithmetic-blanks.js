...
function generateArithmeticLine(label, operation, layout, blankPosition, minimum, maximum, random) {
    ...
    if (layout === 'twoExpressions') {
        const target = operation === '+' ? first + second : first - second;
        const rightPair = operation === '+'
            ? (() => {
                const rightFirst = randomInt(minimum, target - minimum, random);
                return [rightFirst, target - rightFirst];
            })()
            : (() => {
                const rightSecond = randomInt(minimum, maximum - target, random);
