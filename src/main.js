        ...
        renderTemplateForm(editIndex) {
            ...
            box.innerHTML = `<section class="template-editor" aria-labelledby="template-editor-title">
              ...
              <div class="template-editor__section"><h4>1. Thông tin áp dụng</h4><div class="template-editor__fields">
                ...
                <label class="template-editor__field"><span>Loại câu hỏi</span><select id="template-question-type" class="form-input"><option value="Trắc nghiệm" ${!isMatching && (existing?.question_type || 'Trắc nghiệm') === 'Trắc nghiệm' ? 'selected' : ''}>Trắc nghiệm</option><option value="Điền khuyết" ${!isMatching && existing?.question_type === 'Điền khuyết' ? 'selected' : ''}>Điền khuyết</option><option value="So sánh" ${!isMatching && existing?.question_type === 'So sánh' ? 'selected' : ''}>So sánh</option><option value="Đối chiếu trùng khớp" ${isMatching ? 'selected' : ''}>Đối chiếu trùng khớp</option></select></label>
