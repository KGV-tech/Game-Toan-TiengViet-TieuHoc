...
const app = {
    ...
    data: {
        ...
        getKnownStudentClassNames(classlevel = '', currentClassName = '') {
            const targetLevel = this.normalizeClassLevel(classlevel);
            const classNames = (this.users || [])
                .filter(user => user && String(user.role || '').toLowerCase() !== 'admin')
                .filter(user => !targetLevel || this.normalizeClassLevel(user.classlevel) === targetLevel)
                .map(user => this.normalizeClassName(user.class_name))
                .filter(Boolean);
            const current = this.normalizeClassName(currentClassName);
            if (current) classNames.push(current);
            return Array.from(new Set(classNames)).sort((left, right) => left.localeCompare(right, 'vi', { numeric: true, sensitivity: 'base' }));
        },
        getStudentClassLabel(user) {
            const className = this.normalizeClassName(user?.class_name);
    ...
    auth: {
        avatarChoices: {
            ...
            'boy-hoodie': { label: 'Bé trai áo hoodie xanh' },
            'boy-cap': { label: 'Bé trai áo vàng đội mũ' },
            'boy-vest': { label: 'Bé trai đeo kính áo len' },
            'boy-redhair': { label: 'Bé trai tóc đỏ' },
            'boy-goggles': { label: 'Bé trai đeo kính bảo hộ' },
            'girl-ponytail': { label: 'Bé gái tóc đuôi ngựa' },
            'girl-curly': { label: 'Bé gái tóc xoăn áo hoa' },
            'girl-pink-glasses': { label: 'Bé gái kính hồng' },
            'girl-buns': { label: 'Bé gái tóc búi đôi' },
            'girl-sunhat': { label: 'Bé gái đội mũ rộng vành' },
            'boy-wavy': { label: 'Bé trai tóc gợn sóng' },
            'boy-fade': { label: 'Bé trai tóc cắt vạch' },
            'boy-dino': { label: 'Bé trai áo khủng long' },
            'boy-headphones': { label: 'Bé trai đeo tai nghe' },
            'boy-wink': { label: 'Bé trai áo khoác lông' },
            'girl-flower': { label: 'Bé gái kẹp hoa' },
            'girl-bow': { label: 'Bé gái nơ hồng' },
            'girl-star': { label: 'Bé gái kẹp ngôi sao' },
            'girl-bunny': { label: 'Bé gái kẹp thỏ trắng' },
            'girl-streak': { label: 'Bé gái tóc highlight tím' },
        ...
        async register() {
            ...
            const className = app.data.normalizeClassName(document.getElementById('reg-class-name')?.value || '');
            ...
            if (!['1', '2', '3', '4', '5'].includes(cl)) return this.showAuthFeedback('register-error', 'Cấp lớp không hợp lệ.', ['reg-class']);
            if (className) return this.showAuthFeedback('register-error', 'Lớp cụ thể sẽ do giáo viên phân từ danh sách lớp có sẵn.', ['reg-class-name']);
            if (!['', 'male', 'female'].includes(gender || '')) return this.showAuthFeedback('register-error', 'Giới tính không hợp lệ.', ['reg-gender']);
    ...
    game: {
        ...
        loadQuestion() {
            ...
            if (practiceStatus) {
                const label = document.createElement('span');
                const isExamPractice = Boolean(this.state.examName);
                const subjectName = this.state.subject === 'vietnamese' ? 'TIẾNG VIỆT' : 'TOÁN';
                label.textContent = `Bạn đang làm bài ${isExamPractice ? 'Luyện Đề' : 'Luyện tập'}`;
    ...
    admin: {
        ...
        getComposerModuleMeta(module) {
            return {
                templates: {
                    ...
                    openLabel: 'Mở kho Cấu hình Câu hỏi',
                    actionLabel: 'Tạo cấu hình câu hỏi'
                },
                questions: {
        ...
        normalizeExamQuestionStructure(question) {
            ...
            if (copy.answerMode === 'single') return copy;
        ...
        getExamQuestionStructureKind(question) {
            ...
            const supportedAnswerParts = app.data.getValidPartAnswerCounts(question, answerCount);
            const hasExplicitSingleAnswerGroup = Array.isArray(question?.partAnswerCounts)
                && question.partAnswerCounts.length === 1
                && Array.isArray(supportedAnswerParts)
                && supportedAnswerParts.length === 1
                && question?.answerMode !== 'subquestions';
            if (hasExplicitSingleAnswerGroup) return '';
            const hasExplicitAnswerParts = Array.isArray(question?.partAnswerCounts)
                && question.partAnswerCounts.length > 0
                && Array.isArray(supportedAnswerParts)
                && supportedAnswerParts.length === question.partAnswerCounts.length;
            ...
            if ((hasExplicitAnswerParts || answerCount === 4) && supportsGenericAnswerParts) return 'answerParts';
            return '';
        },
        hasSupportedExamPartStructure(question) {
            ...
            const structuredPartCount = ['statements', 'subquestions', 'angleItems', 'angleCountRows', 'sequenceRounds', 'practiceRows', 'comparisonRows']
                .map(key => Array.isArray(question?.[key]) ? question[key].length : 0)
                .find(Boolean) || 0;
            const isExplicitSingleAnswerGroup = Array.isArray(question?.partAnswerCounts)
                && question.partAnswerCounts.length === 1
                && Array.isArray(groupedParts)
                && groupedParts.length === 1
                && question?.answerMode !== 'subquestions';
            const isSingleQuestion = question?.answerMode === 'single'
                || isExplicitSingleAnswerGroup
                || (structuredPartCount === 1 && question?.answerMode !== 'subquestions');
            if (isSingleQuestion) {
                return Array.isArray(groupedParts)
                    && groupedParts.length === 1
                    && !app.data.validateQuestionScoring(question);
            }
            ...
            const partCount = parts ? parts.length : (groupedParts?.length || answerCount);
            if (!this.getSupportedPartCounts(partCount).includes(partCount)) return false;
            if (!groupedParts && answerCount !== partCount) return false;
        ...
        renderPersonalQuests(box) {
            ...
            const assignLabel = quest => {
                if (quest.assign_type === 'class') return `Lớp ${esc(app.data.normalizeClassName(quest.assign_target) || '—')}`;
        ...
        getTeamCompetitionStudents(classlevel, className = '') {
            const cls = app.data.normalizeClassLevel(classlevel);
            const section = app.data.normalizeClassName(className);
            const students = (app.data.users || []).filter(user => {
                if (String(user.role || '').toLowerCase() === 'admin' || user.approved === false) return false;
                if (cls && app.data.normalizeClassLevel(user.classlevel) !== cls) return false;
                return !section || app.data.normalizeClassName(user.class_name) === section;
            });
        ...
        getTeamCompetitionClassNames(classlevel) {
            return Array.from(new Set(this.getTeamCompetitionStudents(classlevel)
                .map(user => app.data.normalizeClassName(user.class_name)).filter(Boolean)))
        ...
        renderTeamCompetitionForm(editId = null) {
            ...
            const className = app.data.normalizeClassName(draft.className);
            ...
            const teamCount = Math.min(8, Math.max(2, Number(draft.teamCount || draft.teams?.length || 2)));
            ...
            const studentLabel = student => `${student.fullname || student.username}${app.data.normalizeClassName(student.class_name) ? ` · ${app.data.normalizeClassName(student.class_name)}` : ''} (${student.username})`;
            ...
            box.innerHTML = `<section class="team-competition-form team-competition-form--new" aria-label="Soạn trận thi đua nhóm">
                ...
                <section class="team-form-section team-form-section--identity"><div class="team-section-heading"><div><span class="team-section-kicker">01 · Thông tin trận</span><h4>Khung trận</h4><p>Đặt ngữ cảnh để cô nhận ra trận ngay khi vào lớp; có thể loại học sinh vắng mặt trước khi chia nhóm.</p></div><span class="team-section-icon" aria-hidden="true">✦</span></div><div class="team-form-grid">
                  ...
                  <label class="team-field-label"><span>Số lượng nhóm</span><input id="team-comp-team-count" class="form-input" type="number" min="2" max="8" value="${teamCount}" onchange="app.admin.switchTeamCompetitionMode()"></label>
        ...
        renderTeamCompetitionBoard(box, id) {
            ...
            const cards = match.teams.map((team, index) => {
                ...
                const roster = !isLive && match.status !== app.teamCompetition.STATUS.ENDED
                    ? `<div class="team-board-members team-board-members--roster"><p class="team-board-members__leader"><span>Trưởng nhóm</span><strong>${leaderName}</strong></p><p class="team-board-members__list"><span>Thành viên</span><strong>${memberNames.join(' · ') || 'Chưa phân công'}</strong></p></div>`
                    : '';
        ...
        getQuestAssignableClasses(classlevel = document.getElementById('quest-target-classlevel')?.value || '') {
            return Array.from(new Set(this.getQuestAssignableStudents(classlevel)
                .map(user => app.data.normalizeClassName(user.class_name))
                .filter(Boolean)))
                .sort((left, right) => left.localeCompare(right, 'vi', { numeric: true, sensitivity: 'base' }));
        },
        updateQuestAssignmentTarget() {
            ...
            if (type === 'class') {
                ...
                const counts = students.reduce((result, student) => {
                    const className = app.data.normalizeClassName(student.class_name);
      ...
      `;
        ...
        getTemplateAnswerMode(template) {
            if (/^word\.three_steps_(relation_total|purchase_total|divide_compare|remaining|ratio_total|legs_constraint|animal_total)_(mcq|fill)$/.test(String(template?.generator_key || ''))) return 'single';
            const configured = String(template?.config?.answerMode || '').trim();
            if (configured === 'single' || configured === 'subquestions') return configured;
            const selectedParts = template?.config?.selectedParts;
            return Array.isArray(selectedParts) && selectedParts.length === 1 ? 'single' : 'subquestions';
        },
        ...
        renderTemplates(box) {
            ...
            const cardMarkup = ({ item, index }) => {
                ...
                const name = item.name || item.generator_key || 'Cấu hình câu hỏi chưa đặt tên';
                ...
                return `<article class="template-library-card template-library-card--${tone}" data-template-card-index="${index}">
                  ...
                  <footer class="template-library-card__actions"><button type="button" class="template-library-card__action template-library-card__action--edit" onclick="app.admin.renderTemplateForm(${index})">Sửa cấu hình câu hỏi</button><button type="button" class="template-library-card__action template-library-card__action--delete" onclick="app.admin.deleteTemplate(${index})">Xóa</button></footer>
                </article>`;
            };
            const emptyMarkup = `<div class="template-library__empty admin-empty-state" role="status"><span class="template-library__empty-icon" aria-hidden="true">✦</span><h4>${templates.length ? 'Không có cấu hình câu hỏi phù hợp' : 'Kho cấu hình câu hỏi đang chờ mẫu đầu tiên'}</h4><p>${templates.length ? 'Thử đổi bộ lọc để xem thêm cấu hình.' : 'Tạo một cấu hình câu hỏi mới để bắt đầu xây ngân hàng câu hỏi theo từng Bài học.'}</p><button type="button" class="template-library__create template-library__create--empty" onclick="app.admin.renderTemplateForm(null)">＋ Tạo cấu hình câu hỏi mới</button></div>`;

            box.innerHTML = `
              <section class="template-library" aria-labelledby="template-library-title">
                <header class="template-library__header">
                  <div><p class="template-library__eyebrow">01 · MẪU SINH CÂU</p><h3 id="template-library-title">Kho Cấu hình Câu hỏi</h3><p>Chọn một thẻ để chỉnh cấu hình, xem Preview hoặc tạo mẫu mới theo đúng Bài học.</p></div>
                  <div class="template-library__header-actions"><span class="template-library__count" role="status">${visible.length}/${templates.length} mẫu</span><button type="button" id="btn-template-create" class="template-library__create" onclick="app.admin.renderTemplateForm(null)">＋ Tạo cấu hình câu hỏi mới</button></div>
                </header>
                ...
                <div class="template-library__result-heading"><div><strong>Hiển thị ${visible.length} cấu hình câu hỏi</strong><span> · ${visible.length === templates.length ? 'Đang xem toàn bộ kho' : `đã lọc từ ${templates.length} mẫu`}</span></div><span class="template-library__hint">Mỗi thẻ gắn với một Bài học cụ thể</span></div>
        ...
        getNewTemplateDraft() {
            ...
            return {
                ...
                config: { minimum: 10000, maximum: 99999, minimumDigits: 5, maximumDigits: 5, allowedPlaces: ['tens', 'hundreds'], allowedDigits: [1, 2, 3, 4, 5, 6, 7, 8, 9], answerMode: 'subquestions', subquestionCount: 4 },
                is_active: true
            };
        },
        renderTemplateForm(editIndex) {
            ...
            if (!existing) {
                alert('Hãy chọn một cấu hình câu hỏi có sẵn để sửa hoặc lưu thành bản mới.');
            ...
            const selectedTemplatePartIndexes = selectedTemplateAnswerMode === 'single'
                ? [0]
                : this.getSelectedPartIndexes(existing, 4);
            this.templateAnswerMode = selectedTemplateAnswerMode;
            ...
            const phase2SequenceBlankMin = Number(config.blankCountMin ?? 1);
            const phase2SequenceBlankMax = Number(config.blankCountMax ?? 3);
            ...
            const phase7ReviewSkills = Array.isArray(config.skills) && config.skills.length ? config.skills : ['b27', 'b28', 'b29', 'b30'];
            const phase8ReviewGroups = Array.isArray(config.groups) && config.groups.length ? config.groups : ['numbers', 'addSub', 'geometry', 'measurement'];
            ...
            const measurementCenturyStart = Number(config.centuryStart ?? 18);
            ...
            box.innerHTML = `<section class="template-editor" aria-labelledby="template-editor-title">
              <header class="template-editor__header"><div><p class="template-editor__eyebrow">KHO CẤU HÌNH CÂU HỎI · LỚP 4</p><h3 id="template-editor-title">${isNew ? 'Tạo cấu hình câu hỏi mới' : 'Sửa cấu hình câu hỏi'}</h3><p>${isNew ? 'Tạo một cấu hình mới để sinh câu hỏi sát từng Bài học.' : 'Chỉnh cấu hình hiện có, xem Preview hoặc lưu thành bản mới để áp dụng cho lớp/chủ đề khác.'}</p></div><span class="template-editor__badge">Câu hỏi động</span></header>
            ...
            const phase4TemplateOptions = [
                ['number.six_digit_numbers', 'Bài 10 · Lập và đọc số sáu chữ số'],
                ['number.million_class', 'Bài 12 · Các số trong phạm vi lớp triệu'],
                ['number.round_hundred_thousands', 'Bài 13 · Làm tròn đến hàng trăm nghìn'],
                ['number.hk1_review_b10_b15', 'Bài 16 · Ôn tập số có nhiều chữ số']
            ];
            phase4TemplateOptions.forEach(([value, label]) => {
                if (generatorControl && !generatorControl.querySelector(`option[value="${value}"]`)) generatorControl.insertAdjacentHTML('beforeend', `<option value="${value}">${label}</option>`);
            });
            const phase5TemplateOptions = [
                ['measurement.practice_cards', 'Bài 20 · Thực hành đọc phiếu đo'],
                ['measurement.hk1_review_b17_b20', 'Bài 21 · Ôn tập đo lường']
            ];
            ...
            const phase7TemplateOptions = [
                ['g4-m-perpendicular-identify', 'Bài 27 · Nhận biết vuông góc'],
                ['g4-m-perpendicular-grid-practice', 'Bài 28 · Thực hành vuông góc trên lưới'],
                ['g4-m-parallel-identify', 'Bài 29 · Nhận biết song song'],
                ['g4-m-parallel-grid-practice', 'Bài 30 · Thực hành song song trên lưới'],
                ['g4-m-quad-classify', 'Bài 31 · Hình bình hành, hình thoi'],
                ['geometry.hk1_review_b27_b31', 'Bài 32 · Ôn tập hình học']
            ];
            phase7TemplateOptions.forEach(([value, label]) => {
                if (generatorControl && !generatorControl.querySelector(`option[value="${value}"]`)) generatorControl.insertAdjacentHTML('beforeend', `<option value="${value}">${label}</option>`);
            });
            const phase8TemplateOptions = [
                ['number.hk1_review_b33_numbers', 'Bài 33 · Ôn tập các số đến lớp triệu'],
                ['number.hk1_review_b34_add_sub', 'Bài 34 · Ôn tập phép cộng, phép trừ'],
                ['geometry.hk1_review_b35', 'Bài 35 · Ôn tập hình học'],
                ['measurement.hk1_review_b36', 'Bài 36 · Ôn tập đo lường'],
                ['number.hk1_review_b37_full', 'Bài 37 · Ôn tập chung']
            ];
            phase8TemplateOptions.forEach(([value, label]) => {
                if (generatorControl && !generatorControl.querySelector(`option[value="${value}"]`)) generatorControl.insertAdjacentHTML('beforeend', `<option value="${value}">${label}</option>`);
            });
        ...
        templatePresets: {
                ...
                'number.four_operations_fill_blanks': {
                    defaultPrompt: '{question}',
                    guide: 'Tạo 4 ý a–d cùng một phép tính được chọn từ pool cộng, trừ, nhân, chia. Ô trống được bốc ở số thứ nhất, số thứ hai hoặc kết quả; mỗi ý đúng được 0,25 điểm. Các lượt khác nhau vẫn có thể đổi phép tính.',
                ...
                'number.four_operations_expressions': {
                    defaultPrompt: '{question}',
                    guide: 'Tạo 4 ý a–d cùng một phép tính được chọn từ pool cộng, trừ, nhân, chia để tính giá trị biểu thức nhiều bước, có ngoặc khi cần. Mỗi ý đúng được 0,25 điểm; các lượt khác nhau vẫn có thể đổi phép tính.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên câu dẫn cùng 4 ý; hoặc chèn <code>{exercises}</code> để tự soạn câu dẫn riêng.',
                ...
                'number.four_arithmetic_blanks': {
                    ...
                    guide: 'Tạo bốn ý a–d cùng một phép tính, một cách đặt hai vế và một vị trí ô trống được chọn từ các pool tương ứng. Có thể chọn cộng, trừ, nhân, chia; phép chia luôn cho kết quả nguyên.',
                ...
                'number.four_arithmetic_comparisons': {
                    ...
                    guide: 'Tạo bốn ý a–d cùng một phép tính và một cách đặt hai vế để kéo dấu so sánh vào chỗ trống. Các dấu >, <, = chỉ là đáp án của cùng dạng so sánh; phép tính được chọn từ pool cộng, trừ, nhân hoặc chia.',
                    hint: 'Dùng <code>{exercises}</code> để chèn trọn 4 dòng, hoặc <code>{question}</code> để giữ nguyên toàn bộ câu hỏi mặc định.',
                ...
                'number.hk1_review_b22_b25': {
                    defaultPrompt: '{question}', guide: 'Tạo một lượt ôn tập Bài 26; mỗi lượt chọn một kỹ năng trong Bài 22–Bài 25 và sinh bốn câu con cùng kỹ năng đó.', hint: 'Dùng <code>{question}</code> để giữ nguyên bốn câu con cùng kỹ năng do game sinh.', preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu con cùng kỹ năng cộng/trừ do game sinh'], ['{skills}', 'phạm vi kỹ năng Bài 22–Bài 25 được phép chọn']]
                },
                'g4-m-perpendicular-identify': {
                    defaultPrompt: '{question}', guide: 'Bài 27 · Tạo bốn hình để nhận biết hai đường thẳng vuông góc bằng giao điểm và góc vuông.', hint: 'Dùng <code>{question}</code> để giữ nguyên bốn hình và phương án do game sinh.', preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn hình nhận biết vuông góc do game sinh']]
                },
                'g4-m-perpendicular-grid-practice': {
                    defaultPrompt: '{question}', guide: 'Bài 28 · Tạo bốn bài thực hành nhận biết đường thẳng vuông góc trên lưới ô vuông.', hint: 'Dùng <code>{question}</code> để giữ nguyên lưới và phương án do game sinh.', preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn bài thực hành vuông góc trên lưới']]
                },
                'g4-m-parallel-identify': {
                    defaultPrompt: '{question}', guide: 'Bài 29 · Tạo bốn hình để nhận biết hai đường thẳng song song bằng hướng và khoảng cách.', hint: 'Dùng <code>{question}</code> để giữ nguyên bốn hình và phương án do game sinh.', preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn hình nhận biết song song do game sinh']]
                },
                'g4-m-parallel-grid-practice': {
                    defaultPrompt: '{question}', guide: 'Bài 30 · Tạo bốn bài thực hành nhận biết đường thẳng song song trên lưới ô vuông.', hint: 'Dùng <code>{question}</code> để giữ nguyên lưới và phương án do game sinh.', preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn bài thực hành song song trên lưới']]
                },
                'g4-m-quad-classify': {
                    defaultPrompt: '{question}', guide: 'Bài 31 · Tạo bốn tứ giác để nhận biết hình bình hành, hình thoi và hình thang.', hint: 'Chọn loại hình được phép sinh; game luôn giữ bốn phương án phân loại nhất quán.', preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn hình tứ giác và phương án do game sinh']]
                },
                'geometry.hk1_review_b27_b31': {
                    defaultPrompt: '{question}', guide: 'Bài 32 · Mỗi lượt chọn một kỹ năng trong Bài 27–Bài 31 và sinh bốn câu con cùng kỹ năng đó.', hint: 'Chọn một hoặc nhiều kỹ năng làm phạm vi; game giữ một kỹ năng thống nhất trong từng lượt.', preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu con cùng kỹ năng hình học do game sinh'], ['{skills}', 'phạm vi kỹ năng Bài 27–Bài 31 được phép chọn']]
                },
                'number.hk1_review_b33_numbers': {
                    defaultPrompt: '{question}', guide: 'Bài 33 · Mỗi lượt chọn một kỹ năng trong Bài 10–Bài 13 và sinh bốn câu con cùng kỹ năng đó.', hint: 'Chọn một hoặc nhiều kỹ năng làm phạm vi; game giữ một kỹ năng thống nhất trong từng lượt.', preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu con cùng kỹ năng số học do game sinh'], ['{skills}', 'phạm vi kỹ năng Bài 10–Bài 13 được phép chọn']]
                },
                'number.hk1_review_b34_add_sub': {
                    defaultPrompt: '{question}', guide: 'Bài 34 · Mỗi lượt chọn một kỹ năng trong Bài 22–Bài 25 và sinh bốn câu con cùng kỹ năng đó.', hint: 'Chọn một hoặc nhiều kỹ năng làm phạm vi; game giữ một kỹ năng thống nhất trong từng lượt.', preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu con cùng kỹ năng cộng/trừ do game sinh'], ['{skills}', 'phạm vi kỹ năng Bài 22–Bài 25 được phép chọn']]
                },
                'geometry.hk1_review_b35': {
                    defaultPrompt: '{question}', guide: 'Bài 35 · Mỗi lượt chọn một kỹ năng trong Bài 27–Bài 30 và sinh bốn câu con cùng kỹ năng đó.', hint: 'Chọn một hoặc nhiều kỹ năng làm phạm vi; hình minh họa được giữ trong bốn câu con cùng dạng.', preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu con cùng kỹ năng hình học do game sinh'], ['{skills}', 'phạm vi kỹ năng Bài 27–Bài 30 được phép chọn']]
                },
                'measurement.hk1_review_b36': {
                    defaultPrompt: '{question}', guide: 'Bài 36 · Mỗi lượt chọn một kỹ năng trong Bài 17–Bài 20 và sinh bốn câu con cùng kỹ năng đó.', hint: 'Chọn một hoặc nhiều kỹ năng làm phạm vi; game giữ một kỹ năng thống nhất trong từng lượt.', preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu con cùng kỹ năng đo lường do game sinh'], ['{skills}', 'phạm vi kỹ năng Bài 17–Bài 20 được phép chọn']]
                },
                'number.hk1_review_b37_full': {
                    defaultPrompt: '{question}', guide: 'Bài 37 · Mỗi lượt chọn một nhóm kiến thức Học kỳ 1 và sinh bốn câu con cùng nhóm đó.', hint: 'Chọn một hoặc nhiều nhóm làm phạm vi; game giữ một nhóm thống nhất trong từng lượt.', preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu con cùng nhóm kiến thức do game sinh'], ['{groups}', 'phạm vi nhóm kiến thức được phép chọn']]
                },
                ...
                'g4-m-angle-review': {
                    defaultPrompt: '{question}',
                    guide: 'Bài 9 · Mỗi lượt chọn một dạng góc: đọc số đo trên thước hoặc nhận biết loại góc, rồi sinh bốn câu con cùng dạng.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn câu con cùng một dạng góc do game sinh.',
                    preview: 'live',
                    type: 'Trắc nghiệm',
                    variables: [['{question}', 'hướng dẫn và bốn câu con cùng dạng do game sinh'], ['{skills}', 'phạm vi dạng góc được phép chọn']]
                },
                ...
                'number.even_odd_sequence': {
                    defaultPrompt: '{question}',
                    guide: 'Tạo bốn dãy số chẵn hoặc lẻ gồm 6 số, tăng đều theo bước nhảy chẵn; mỗi dãy có từ 1 đến 3 ô trống.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn dãy điền khuyết do game sinh.',
                    preview: 'live', type: 'Chuỗi Quy luật',
                ...
                'number.hk1_review_b01_b04': {
                    defaultPrompt: '{question}',
                    guide: 'Tạo bộ ôn tập Bài 6; mỗi lượt chọn một kỹ năng trong Bài 1–4 và sinh đủ bốn câu con cùng kỹ năng đó.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn câu con cùng một kỹ năng.',
                    preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu con cùng kỹ năng do game sinh'], ['{skills}', 'phạm vi kỹ năng được phép chọn']]
                },
                'number.six_digit_numbers': {
                    defaultPrompt: '{question}',
                    guide: 'Bài 10 · Mỗi lượt chọn một dạng trong phạm vi số sáu chữ số và sinh bốn câu con cùng dạng.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn câu con cùng dạng Bài 10 do game sinh.',
                    preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu cùng dạng Bài 10 do game sinh'], ['{modes}', 'phạm vi dạng được phép chọn']]
                },
                'number.million_class': {
                    defaultPrompt: '{question}',
                    guide: 'Bài 12 · Mỗi lượt chọn một dạng đọc, viết, xác định chữ số hoặc phân tích số trong lớp triệu và sinh bốn câu con cùng dạng.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn câu con cùng dạng lớp triệu do game sinh.',
                    preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu cùng dạng lớp triệu do game sinh'], ['{modes}', 'phạm vi dạng được phép chọn']]
                },
                'number.round_hundred_thousands': {
                    defaultPrompt: '{question}',
                    guide: 'Bài 13 · Mỗi lượt chọn dạng làm tròn số hoặc nhận biết quy tắc, rồi sinh bốn câu con cùng dạng.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn câu con cùng dạng làm tròn do game sinh.',
                    preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu cùng dạng Bài 13 do game sinh'], ['{modes}', 'phạm vi dạng được phép chọn']]
                },
                'number.hk1_review_b10_b15': {
                    defaultPrompt: '{question}',
                    guide: 'Bài 16 · Mỗi lượt chọn một kỹ năng trong Bài 10–15 và sinh bốn câu con cùng kỹ năng đó.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn câu con cùng kỹ năng do game sinh.',
                    preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu con cùng kỹ năng do game sinh'], ['{skills}', 'phạm vi kỹ năng được phép chọn']]
                },
                'measurement.mass_unit_convert': {
                    defaultPrompt: '{question}',
                    guide: 'Bài 17 · Đổi các số đo khối lượng giữa yến, tạ, tấn và ki-lô-gam bằng bốn câu con.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn câu đổi khối lượng và ô điền do game sinh.',
                    preview: 'live', type: 'Điền khuyết',
                    variables: [['{question}', 'toàn bộ bốn câu đổi khối lượng do game sinh']]
                },
                'measurement.area_unit_convert': {
                    defaultPrompt: '{question}',
                    guide: 'Bài 18 · Đổi các số đo diện tích giữa m², dm², cm² và mm² bằng bốn câu con.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn câu đổi diện tích và ô điền do game sinh.',
                    preview: 'live', type: 'Điền khuyết',
                    variables: [['{question}', 'toàn bộ bốn câu đổi diện tích do game sinh']]
                },
                'measurement.time_unit_convert': {
                    defaultPrompt: '{question}',
                    guide: 'Bài 19 · Đổi các số đo thời gian giữa phút, giây, giờ, tuần và ngày bằng bốn câu con.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn câu đổi thời gian và ô điền do game sinh.',
                    preview: 'live', type: 'Điền khuyết',
                    variables: [['{question}', 'toàn bộ bốn câu đổi thời gian do game sinh']]
                },
                'measurement.compare_units': {
                    defaultPrompt: '{question}',
                    guide: 'Tạo bốn phép so sánh số đo cùng loại sau khi đổi về cùng một đơn vị.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn dòng so sánh và dấu kéo thả do game sinh.',
                    preview: 'live', type: 'Kéo thả',
                    variables: [['{question}', 'toàn bộ bốn phép so sánh do game sinh']]
                },
                'measurement.match_equivalences': {
                    defaultPrompt: '{question}',
                    guide: 'Tạo bài nối số đo với giá trị tương đương; một lựa chọn được dùng làm nhiễu.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên hai cột số đo do game sinh.',
                    preview: 'live', type: 'Đối chiếu trùng khớp',
                    variables: [['{question}', 'toàn bộ bài nối hai cột do game sinh']]
                },
                'measurement.unit_true_false': {
                    defaultPrompt: '{question}',
                    guide: 'Tạo bốn nhận định Đúng/Sai về các phép đổi đơn vị khối lượng, diện tích, thời gian và thế kỉ.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn nhận định Đúng/Sai do game sinh.',
                    preview: 'live', type: 'Đúng/Sai',
                    variables: [['{question}', 'toàn bộ bốn nhận định Đúng/Sai do game sinh']]
                },
                'measurement.century_identification': {
                    defaultPrompt: '{question}',
                    guide: 'Bài 19 · Xác định thế kỉ của bốn năm được sinh trong khoảng cấu hình.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn câu xác định thế kỉ và phương án do game sinh.',
                    preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu xác định thế kỉ do game sinh']]
                },
                'measurement.word_problem_units': {
                    defaultPrompt: '{question}',
                    guide: 'Tạo bốn bài toán lời văn về khối lượng, diện tích hoặc thời gian; học sinh đổi đơn vị rồi tính.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn bài toán và ô điền đáp số do game sinh.',
                    preview: 'live', type: 'Điền khuyết',
                    variables: [['{question}', 'toàn bộ bốn bài toán lời văn do game sinh']]
                },
                'measurement.practice_cards': {
                    defaultPrompt: '{question}',
                    guide: 'Bài 20 · Mỗi lượt chọn một nhóm đơn vị và tạo bốn phiếu đo cùng nhóm. Dữ liệu và đáp án được sinh mới ở mỗi lượt.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn phiếu đo cùng nhóm và các phương án do game sinh.',
                    preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn phiếu đo cùng nhóm do game sinh'], ['{kinds}', 'phạm vi nhóm đơn vị được phép chọn']]
                },
                'measurement.hk1_review_b17_b20': {
                    defaultPrompt: '{question}',
                    guide: 'Bài 21 · Mỗi lượt chọn một kỹ năng trong Bài 17–20 và sinh bốn câu con cùng kỹ năng đó.',
                    hint: 'Dùng <code>{question}</code> để giữ nguyên bốn câu con cùng kỹ năng do game sinh.',
                    preview: 'live', type: 'Trắc nghiệm',
                    variables: [['{question}', 'toàn bộ bốn câu con cùng kỹ năng đo lường do game sinh'], ['{skills}', 'phạm vi kỹ năng được phép chọn']]
                },
        ...
        renderTemplatePreview(generator) {
            ...
            if (generator === 'number.hk1_review_b01_b04') return preview('Luyện tập Bài 3 · Nhận biết số chẵn, số lẻ:', `<div class="template-preview__mc">${['Số nào là số lẻ?', 'Số nào là số lẻ?', 'Số nào là số lẻ?', 'Số nào là số lẻ?'].map((prompt, index) => `<div><b>${'abcd'[index]})</b><small>Cùng dạng Bài 3</small>${prompt}${choices(['2 408', '3 517', '6 824', '9 130'])}</div>`).join('')}</div>`, 'template-preview--multiple-choice');
            if (generator === 'measurement.practice_cards') return preview('Thực hành đổi đơn vị khối lượng:', `<div class="template-preview__mc">${[
                ['2 tấn 5 tạ = ? kg', ['2 050', '2 500', '2 700', '2 750']],
                ['3 tấn 4 tạ = ? kg', ['3 040', '3 400', '3 500', '3 900']],
                ['1 tấn 8 tạ = ? kg', ['1 080', '1 800', '2 000', '2 800']],
                ['4 tấn 2 tạ = ? kg', ['4 020', '4 200', '4 800', '5 000']]
            ].map(([prompt, values], index) => `<div><b>${'abcd'[index]})</b>${prompt}${choices(values)}</div>`).join('')}</div>`, 'template-preview--multiple-choice');
            if (generator === 'measurement.hk1_review_b17_b20') return preview('Luyện tập Bài 17 · Khối lượng:', `<div class="template-preview__mc">${[
                ['7 tạ = ? kg', ['70', '700', '7 000', '70 000']],
                ['3 tấn 4 tạ = ? kg', ['3 040', '3 400', '3 500', '3 900']],
                ['2 tấn 6 tạ = ? kg', ['2 060', '2 600', '2 800', '3 200']],
                ['5 tạ 8 yến = ? kg', ['58', '580', '5 080', '5 800']]
            ].map(([prompt, values], index) => `<div><b>${'abcd'[index]})</b>${prompt}${choices(values)}</div>`).join('')}</div>`, 'template-preview--multiple-choice');
            if (generator === 'number.hk1_review_b22_b25') return preview('Luyện tập Bài 22 · Phép cộng:', `<div class="template-preview__mc">${[
                ['Tính: 24 580 + 13 420 = ?', ['37 000', '38 000', '39 000', '40 000']],
                ['Tính: 15 625 + 21 375 = ?', ['36 000', '37 000', '38 000', '39 000']],
                ['Tính: 42 708 + 16 292 = ?', ['58 000', '59 000', '60 000', '61 000']],
                ['Tính: 31 450 + 8 550 = ?', ['39 000', '40 000', '41 000', '42 000']]
            ].map(([prompt, values], index) => `<div><b>${'abcd'[index]})</b>${prompt}${choices(values)}</div>`).join('')}</div>`, 'template-preview--multiple-choice');
            if (['g4-m-perpendicular-identify', 'g4-m-perpendicular-grid-practice', 'g4-m-parallel-identify', 'g4-m-parallel-grid-practice'].includes(generator)) return preview('Quan sát hình và chọn mối quan hệ đúng:', `<div class="template-preview__mc">${['Hình 1 · hai đường thẳng', 'Hình 2 · hai đường thẳng', 'Hình 3 · hai đường thẳng', 'Hình 4 · hai đường thẳng'].map((prompt, index) => `<div><b>${'abcd'[index]})</b><span class="template-preview__geometry-placeholder" aria-hidden="true">╱ ╲</span>${prompt}${choices(generator.includes('parallel') ? ['Song song', 'Không song song'] : ['Vuông góc', 'Không vuông góc'])}</div>`).join('')}</div>`, 'template-preview--multiple-choice');
            if (generator === 'g4-m-quad-classify') return preview('Nhận biết hình bình hành và hình thoi:', `<div class="template-preview__mc">${['Hình bình hành', 'Hình thoi', 'Hình chữ nhật', 'Hình thang'].map((shape, index) => `<div><b>${'abcd'[index]})</b><span class="template-preview__geometry-placeholder template-preview__geometry-placeholder--quad" aria-hidden="true">◇</span>${shape}${choices(['Hình bình hành', 'Hình thoi', 'Cả hình bình hành và hình thoi', 'Không phải cả hai'])}</div>`).join('')}</div>`, 'template-preview--multiple-choice');
            if (generator === 'geometry.hk1_review_b27_b31') return preview('Luyện tập Bài 27 · Hai đường thẳng vuông góc:', `<div class="template-preview__mc">${['Hình 1 · Hai đường thẳng có vuông góc không?', 'Hình 2 · Hai đường thẳng có vuông góc không?', 'Hình 3 · Hai đường thẳng có vuông góc không?', 'Hình 4 · Hai đường thẳng có vuông góc không?'].map((prompt, index) => `<div><b>${'abcd'[index]})</b><span class="template-preview__geometry-placeholder" aria-hidden="true">◇</span>${prompt}${choices(['Vuông góc', 'Không vuông góc'])}</div>`).join('')}</div>`, 'template-preview--multiple-choice');
            if (generator === 'number.hk1_review_b33_numbers') return preview('Luyện tập Bài 10 · Lập số sáu chữ số:', `<div class="template-preview__mc">${['Số gồm 1 trăm nghìn, 2 chục nghìn, 5 nghìn là số nào?', 'Số gồm 2 trăm nghìn, 1 chục nghìn, 5 nghìn là số nào?', 'Số gồm 5 trăm nghìn, 1 chục nghìn, 2 nghìn là số nào?', 'Số gồm 5 trăm nghìn, 2 chục nghìn, 1 nghìn là số nào?'].map((prompt, index) => `<div><b>${'abcd'[index]})</b>${prompt}${choices(['125 000', '215 000', '512 000', '521 000'])}</div>`).join('')}</div>`, 'template-preview--multiple-choice');
            if (generator === 'number.hk1_review_b34_add_sub') return preview('Luyện tập Bài 22 · Phép cộng:', `<div class="template-preview__mc">${['24 580 + 13 420 = ?', '15 625 + 21 375 = ?', '42 708 + 16 292 = ?', '31 450 + 8 550 = ?'].map((prompt, index) => `<div><b>${'abcd'[index]})</b>${prompt}${choices(['37 000', '38 000', '39 000', '40 000'])}</div>`).join('')}</div>`, 'template-preview--multiple-choice');
            if (generator === 'geometry.hk1_review_b35') return preview('Luyện tập Bài 27 · Hai đường thẳng vuông góc:', `<div class="template-preview__mc">${['Hình 1 · Nhận biết vuông góc', 'Hình 2 · Nhận biết vuông góc', 'Hình 3 · Nhận biết vuông góc', 'Hình 4 · Nhận biết vuông góc'].map((prompt, index) => `<div><b>${'abcd'[index]})</b><span class="template-preview__geometry-placeholder" aria-hidden="true">╱ ╲</span>${prompt}${choices(['Vuông góc', 'Không vuông góc'])}</div>`).join('')}</div>`, 'template-preview--multiple-choice');
            if (generator === 'measurement.hk1_review_b36') return preview('Luyện tập Bài 17 · Khối lượng:', `<div class="template-preview__mc">${['7 tạ = ? kg', '3 tấn 4 tạ = ? kg', '2 tấn 6 tạ = ? kg', '5 tạ 8 yến = ? kg'].map((prompt, index) => `<div><b>${'abcd'[index]})</b>${prompt}${choices(['70', '700', '7 000', '70 000'])}</div>`).join('')}</div>`, 'template-preview--multiple-choice');
            if (generator === 'number.hk1_review_b37_full') return preview('Luyện tập Bài 37 · Số và lớp triệu:', `<div class="template-preview__mc">${['Số gồm 1 trăm nghìn, 2 chục nghìn, 5 nghìn là số nào?', 'Số gồm 2 trăm nghìn, 1 chục nghìn, 5 nghìn là số nào?', 'Số gồm 5 trăm nghìn, 1 chục nghìn, 2 nghìn là số nào?', 'Số gồm 5 trăm nghìn, 2 chục nghìn, 1 nghìn là số nào?'].map((prompt, index) => `<div><b>${'abcd'[index]})</b>${prompt}${choices(['125 000', '215 000', '512 000', '521 000'])}</div>`).join('')}</div>`, 'template-preview--multiple-choice');
        ...
        setTemplateAnswerMode(mode) {
            this.templateAnswerMode = mode === 'single' ? 'single' : 'subquestions';
            if (this.templateAnswerMode === 'single') {
                document.querySelectorAll('.template-part-checkbox').forEach(input => { input.checked = Number(input.dataset.partIndex) === 0; });
                this.templatePartSelection = [0];
            } else if (!Array.isArray(this.templatePartSelection) || ![2, 4].includes(this.templatePartSelection.length)) {
                this.setTemplatePartCount(4);
                return;
            }
            this.syncTemplateAnswerModeUI();
            this.syncTemplatePartSelectionUI();
        },
        syncTemplateAnswerModeUI() {
            const mode = this.templateAnswerMode || document.getElementById('template-answer-mode')?.value || 'subquestions';
            const countField = document.getElementById('template-subquestion-count-field');
            const help = document.getElementById('template-answer-mode-help');
            const partSelection = document.querySelector('.template-editor__part-selection');
            const isSingle = mode === 'single';
            if (countField) countField.hidden = isSingle;
            if (help) help.textContent = isSingle
                ? 'Câu 1 ý dùng câu hỏi chung và câu trả lời chung, không tạo câu con.'
                : 'Câu có câu hỏi con gồm 2 hoặc 4 ý; mỗi ý được chia điểm như hiện tại.';
            if (partSelection) partSelection.hidden = isSingle;
        },
        readTemplatePartSelection() {
            if ((this.templateAnswerMode || document.getElementById('template-answer-mode')?.value) === 'single') return [0];
        ...
        syncTemplatePartSelectionUI(message = '') {
            ...
            const pointText = selected.length ? (1 / selected.length).toLocaleString('vi-VN', { maximumFractionDigits: 2 }) : '—';
            const mode = this.templateAnswerMode || document.getElementById('template-answer-mode')?.value || 'subquestions';
            if (status) status.textContent = message || (mode === 'single'
                ? 'Câu 1 ý chung · 1 điểm cho câu trả lời chung'
                : `${selected.length}/4 câu hỏi con đang chọn · ${pointText} điểm/câu`);
            const countSelect = document.getElementById('template-part-count');
            if (countSelect && [2, 4].includes(selected.length)) countSelect.value = String(selected.length);
        ...
        updateTemplatePartSelection() {
            if ((this.templateAnswerMode || document.getElementById('template-answer-mode')?.value) === 'single') {
                this.setTemplateAnswerMode('single');
                return;
            }
            ...
            if (![2, 4].includes(selected.length)) {
                ...
                this.syncTemplatePartSelectionUI('Lựa chọn 2 chỉ nhận 2 hoặc 4 câu hỏi con.');
                return;
            }
        ...
        setTemplatePartCount(count) {
            const targetCount = Number(count);
            if (![2, 4].includes(targetCount)) return;
            const checkboxes = [...document.querySelectorAll('.template-part-checkbox')];
            const current = checkboxes.filter(input => input.checked);
            const chosen = current.slice(0, targetCount);
            ...
            if (modeSelect) modeSelect.value = 'subquestions';
            this.syncTemplateAnswerModeUI();
            this.syncTemplatePartSelectionUI();
        },
        openTemplatePreview() {
        ...
        showTemplateExample() {
            ...
            const isPhase4Review = generator === 'number.hk1_review_b10_b15';
            const measurementTemplateKeys = ['measurement.mass_unit_convert', 'measurement.area_unit_convert', 'measurement.time_unit_convert', 'measurement.compare_units', 'measurement.match_equivalences', 'measurement.unit_true_false', 'measurement.century_identification', 'measurement.word_problem_units', 'measurement.practice_cards', 'measurement.hk1_review_b17_b20'];
            const isMeasurementTemplate = measurementTemplateKeys.includes(generator);
            const phase5TemplateKeys = ['measurement.practice_cards', 'measurement.hk1_review_b17_b20'];
            ...
            const phase7TemplateKeys = ['g4-m-perpendicular-identify', 'g4-m-perpendicular-grid-practice', 'g4-m-parallel-identify', 'g4-m-parallel-grid-practice', 'g4-m-quad-classify', 'geometry.hk1_review_b27_b31'];
            const phase8TemplateKeys = ['number.hk1_review_b33_numbers', 'number.hk1_review_b34_add_sub', 'geometry.hk1_review_b35', 'measurement.hk1_review_b36', 'number.hk1_review_b37_full'];
            const isPhase7Template = phase7TemplateKeys.includes(generator);
            const isPhase7Review = generator === 'geometry.hk1_review_b27_b31';
            ...
            if (phase4DigitDefaults) {
                ...
                if (maximumDigitsControl) maximumDigitsControl.value = phase4DigitDefaults[1];
            }
            ...
            if (arithmeticLegend) arithmeticLegend.textContent = isFourArithmetic ? 'Phép tính được phép (chọn một cho cả bốn ý)' : 'Phép tính có thể bốc';
            ...
            document.querySelectorAll('.template-editor__rule--phase7-controls').forEach(rule => { rule.hidden = !isPhase7Template; });
            document.querySelectorAll('.template-phase7-quad-kinds-fieldset').forEach(rule => { rule.hidden = !isPhase7Quad; });
            document.querySelectorAll('.template-phase7-review-skills-fieldset').forEach(rule => { rule.hidden = !isPhase7Review; });
            document.querySelectorAll('.template-editor__rule--phase8-controls').forEach(rule => { rule.hidden = !isPhase8FullReview; });
            ...
            document.querySelectorAll('.template-measurement-config--time').forEach(rule => { rule.hidden = generator !== 'measurement.time_unit_convert'; });
            document.querySelectorAll('.template-measurement-config--century').forEach(rule => { rule.hidden = generator !== 'measurement.century_identification'; });
            document.querySelectorAll('.template-measurement-config--word-problem').forEach(rule => { rule.hidden = generator !== 'measurement.word_problem_units'; });
            document.querySelectorAll('#template-phase2-minimum, #template-phase2-maximum, #template-phase2-list-length-min, #template-phase2-list-length-max, #template-phase2-digit-count, #template-phase2-sequence-steps, #template-phase2-sequence-blank-min, #template-phase2-sequence-blank-max').forEach(input => { input.disabled = !isPhase2B03; });
            ...
            const answerModeControl = document.getElementById('template-answer-mode');
            if (answerModeControl) {
                answerModeControl.disabled = isB05Template;
                if (isB05Template) {
                    answerModeControl.value = 'single';
                    this.templateAnswerMode = 'single';
                }
            }
            this.syncTemplateAnswerModeUI();
            this.syncTemplateTrueFalseControls();
        ...
        collectTemplateForm() {
            ...
            const isPhase4Review = generatorKey === 'number.hk1_review_b10_b15';
            const measurementTemplateKeys = ['measurement.mass_unit_convert', 'measurement.area_unit_convert', 'measurement.time_unit_convert', 'measurement.compare_units', 'measurement.match_equivalences', 'measurement.unit_true_false', 'measurement.century_identification', 'measurement.word_problem_units', 'measurement.practice_cards', 'measurement.hk1_review_b17_b20'];
            const isMeasurementTemplate = measurementTemplateKeys.includes(generatorKey);
            ...
            const phase7TemplateKeys = ['g4-m-perpendicular-identify', 'g4-m-perpendicular-grid-practice', 'g4-m-parallel-identify', 'g4-m-parallel-grid-practice', 'g4-m-quad-classify', 'geometry.hk1_review_b27_b31'];
            const phase8TemplateKeys = ['number.hk1_review_b33_numbers', 'number.hk1_review_b34_add_sub', 'geometry.hk1_review_b35', 'measurement.hk1_review_b36', 'number.hk1_review_b37_full'];
            const isPhase7Template = phase7TemplateKeys.includes(generatorKey);
            const isPhase7Review = generatorKey === 'geometry.hk1_review_b27_b31';
            const isPhase7Quad = generatorKey === 'g4-m-quad-classify';
            const isPhase8Template = phase8TemplateKeys.includes(generatorKey);
            const isPhase8FullReview = generatorKey === 'number.hk1_review_b37_full';
            ...
            const phase2SequenceBlankMin = Number(value('template-phase2-sequence-blank-min'));
            const phase2SequenceBlankMax = Number(value('template-phase2-sequence-blank-max'));
            ...
            const phase7Shapes = selectedSafeValues('phase7-quad-kinds');
            const phase7ReviewSkills = selectedSafeValues('phase7-review-skills');
            const phase8ReviewGroups = selectedSafeValues('phase8-review-groups');
            ...
            const measurementCenturyEnd = Number(document.getElementById('template-measurement-century-end')?.value || 21);
            ...
            const phase7Config = isPhase7Quad ? { allowedShapes: phase7Shapes } : (isPhase7Review ? { skills: phase7ReviewSkills } : {});
            const phase8SkillDefaults = {
                'number.hk1_review_b33_numbers': ['b10', 'b11', 'b12', 'b13'],
                'number.hk1_review_b34_add_sub': ['b22', 'b23', 'b24', 'b25'],
                'geometry.hk1_review_b35': ['b27', 'b28', 'b29', 'b30'],
            ...
            const measurementConfig = generatorKey === 'measurement.mass_unit_convert'
                ...
                : generatorKey === 'measurement.area_unit_convert'
                    ? { allowedKinds: measurementAreaKinds }
                    : generatorKey === 'measurement.time_unit_convert'
                        ? { allowedKinds: measurementTimeKinds }
                        : generatorKey === 'measurement.century_identification'
                            ? { centuryStart: measurementCenturyStart, centuryEnd: measurementCenturyEnd }
            ...
            if (isPhase2B03) {
                ...
                if (generatorKey === 'number.even_odd_sequence' && (!Number.isInteger(phase2SequenceBlankMin) || !Number.isInteger(phase2SequenceBlankMax) || phase2SequenceBlankMin < 1 || phase2SequenceBlankMax < phase2SequenceBlankMin || phase2SequenceBlankMax > 3)) throw new Error('Dãy chẵn/lẻ chỉ có thể có từ 1 đến 3 ô trống.');
                if (generatorKey === 'number.even_odd_sequence' && phase2SequenceSteps.some(step => phase2Parities.some(parity => {
                    const sequenceMaximum = phase2Maximum - step * 5;
            ...
            if (isPhase4Review && (!phase4Skills.length || phase4Skills.some(skill => !['b10', 'b11', 'b12', 'b13', 'b14', 'b15'].includes(skill)))) {
                throw new Error('Bộ ôn tập Bài 10 đến Bài 15 cần chọn ít nhất một kỹ năng hợp lệ.');
            }
            if (isPhase5Practice && (phase5PracticeKinds.length === 0 || new Set(phase5PracticeKinds).size !== phase5PracticeKinds.length || phase5PracticeKinds.some(kind => !['mass', 'area', 'time', 'century'].includes(kind)))) {
                throw new Error('Bài 20 cần chọn ít nhất một nhóm đơn vị thực hành hợp lệ.');
            }
            if (isPhase5Review && (!phase5ReviewSkills.length || phase5ReviewSkills.some(skill => !['b17', 'b18', 'b19', 'b20'].includes(skill)))) {
                throw new Error('Bộ ôn tập Bài 17 đến Bài 20 cần chọn ít nhất một kỹ năng hợp lệ.');
            }
            ...
            if (isPhase6Review && (!phase6ReviewSkills.length || phase6ReviewSkills.some(skill => !['b22', 'b23', 'b24', 'b25'].includes(skill)))) {
                throw new Error('Bộ ôn tập Bài 22 đến Bài 25 cần chọn ít nhất một kỹ năng hợp lệ.');
            }
            ...
            if (isPhase8FullReview && (!phase8ReviewGroups.length || phase8ReviewGroups.some(group => !['numbers', 'addSub', 'geometry', 'measurement'].includes(group)))) {
                throw new Error('Bài 37 cần chọn ít nhất một nhóm kiến thức HK1 hợp lệ.');
            }
            ...
            if (generatorKey === 'measurement.area_unit_convert' && (!measurementAreaKinds.length || new Set(measurementAreaKinds).size !== measurementAreaKinds.length || measurementAreaKinds.some(kind => !['m2ToDm2', 'dm2ToCm2', 'dm2ToMm2', 'cm2ToDm2'].includes(kind)))) {
                throw new Error('Bài 18 cần chọn ít nhất một dạng đổi diện tích hợp lệ.');
            }
            if (generatorKey === 'measurement.time_unit_convert' && (!measurementTimeKinds.length || new Set(measurementTimeKinds).size !== measurementTimeKinds.length || measurementTimeKinds.some(kind => !['minuteToSeconds', 'hourToMinutes', 'minutesAndSecondsToSeconds', 'weekAndDaysToDays'].includes(kind)))) {
                throw new Error('Bài 19 cần chọn ít nhất một dạng đổi thời gian hợp lệ.');
            }
            if (generatorKey === 'measurement.century_identification' && (!Number.isSafeInteger(measurementCenturyStart) || !Number.isSafeInteger(measurementCenturyEnd) || measurementCenturyStart < 1 || measurementCenturyEnd > 99 || measurementCenturyStart > measurementCenturyEnd || measurementCenturyEnd - measurementCenturyStart + 1 < 4)) {
                throw new Error('Khoảng thế kỉ cần có ít nhất bốn thế kỉ hợp lệ.');
            }
            if (generatorKey === 'measurement.word_problem_units' && (!measurementScenarioKinds.length || new Set(measurementScenarioKinds).size !== measurementScenarioKinds.length || measurementScenarioKinds.some(kind => !['mass', 'area', 'time'].includes(kind)))) {
                throw new Error('Bài toán đơn vị đo cần chọn ít nhất một nhóm tình huống hợp lệ.');
            }
            ...
            if (['number.four_operations_fill_blanks', 'number.four_operations_expressions', 'number.four_arithmetic_blanks', 'number.four_arithmetic_comparisons'].includes(template.generator_key)) {
                ...
                const allowedArithmeticOperations = ['+', '-', '*', '/'];
                if (!arithmeticOperations.length || arithmeticOperations.some(operation => !allowedArithmeticOperations.includes(operation)) || (requiresLayouts && !arithmeticLayouts.length)) throw new Error(requiresLayouts ? 'Hãy chọn ít nhất một phép tính và một dạng hiển thị hai vế.' : 'Hãy chọn ít nhất một phép tính.');
            ...
            if (!isB05Template && answerMode === 'single') {
                template.config.selectedParts = [0];
            } else if (!isB05Template && selectedPartIndexes.length !== 4) {
                template.config.selectedParts = selectedPartIndexes;
            } else {
                delete template.config.selectedParts;
            }
            if (!isB05Template && answerMode === 'subquestions') template.config.subquestionCount = selectedPartIndexes.length;
            else delete template.config.subquestionCount;
        ...
        async saveTemplate(editIndex, asCopy = false) {
            ...
            if (editIndex !== null && !original) return alert('Không tìm thấy cấu hình câu hỏi gốc.');
            ...
            if (window.supabase && (!isUpdate || !String(original.id || '').startsWith('temp_'))) {
                ...
                if (error || !data?.[0]) { alert('Không thể lưu cấu hình câu hỏi trên server. Hãy chạy file SQL tạo bảng trước.'); return; }
        ...
        async deleteTemplate(index) {
            ...
            if (!template || !confirm(`Xóa cấu hình câu hỏi “${template.name}”?`)) return;
            if (window.supabase && !String(template.id || '').startsWith('temp_')) {
                ...
                if (error) { alert('Không thể xóa cấu hình câu hỏi trên server.'); return; }
            }
      ...
      `;
        ...
        autoGenerateExam() {
            ...
            const used = new Set();
            let skippedUnsupportedStructure = false;
            const addUnique = question => {
                 if (!question || typeof question !== 'object') return false;
                 let copy = JSON.parse(JSON.stringify(question));
                 if (requiresSupportedPartStructure) copy = this.normalizeExamQuestionStructure(copy);
                 // Đề Toán lớp 4 chỉ nhận một ý chung hoặc 2/4 câu hỏi con.
                 if (requiresSupportedPartStructure && !this.hasSupportedExamPartStructure(copy)) {
                     skippedUnsupportedStructure = true;
                     return false;
                 }
            ...
            const missingTopics = requiresSupportedPartStructure
            ...
            if (questions.length < app.game.questionsPerRound) {
                const structureHint = skippedUnsupportedStructure
                    ? ' Các câu không có dạng 1 ý chung hoặc 2/4 câu hỏi con đã được bỏ qua.'
                    : '';
        ...
        renderPlayersList(isPending) {
            ...
            const classCount = new Set(allStudents.map(u => app.data.normalizeClassName(u.class_name)).filter(Boolean)).size;
        ...
        showAddPlayerForm(editUsername) {
            ...
            const selectedClassLevel = app.data.normalizeClassLevel(u?.classlevel) || (u ? '' : '5');
            const selectedClassName = app.data.normalizeClassName(u?.class_name);
            const classNameOptions = [
                '<option value="">Không khai báo lớp cụ thể</option>',
                ...app.data.getKnownStudentClassNames(selectedClassLevel, selectedClassName)
                    .map(className => `<option value="${app.data.sanitizeHTML(className)}"${className === selectedClassName ? ' selected' : ''}>Lớp ${app.data.sanitizeHTML(className)}</option>`)
            ].join('');
                ...
                <section class="admin-student-form" aria-label="${u ? 'Sửa thông tin học sinh' : 'Thêm học sinh mới'}">
                    ...
                    <div class="admin-student-form__panel">
                        ...
                        <div class="admin-student-form__grid">
                            ...
                            <label class="admin-student-form__field"><span>Cấp lớp</span><select id="add-class" class="form-input" onchange="app.admin.updateStudentClassNameOptions()"><option value="1" ${selectedClassLevel === '1' ? 'selected' : ''}>Lớp 1</option><option value="2" ${selectedClassLevel === '2' ? 'selected' : ''}>Lớp 2</option><option value="3" ${selectedClassLevel === '3' ? 'selected' : ''}>Lớp 3</option><option value="4" ${selectedClassLevel === '4' ? 'selected' : ''}>Lớp 4</option><option value="5" ${selectedClassLevel === '5' ? 'selected' : ''}>Lớp 5</option></select></label>
                            <label class="admin-student-form__field"><span>Lớp cụ thể</span><select id="add-class-name" class="form-input">${classNameOptions}</select><small>Chỉ chọn lớp đã có trong roster; để trống nếu chưa phân lớp.</small></label>
        ...
        updateStudentClassNameOptions() {
            const classLevelSelect = document.getElementById('add-class');
            const classNameSelect = document.getElementById('add-class-name');
            if (!classLevelSelect || !classNameSelect) return;
            const currentClassName = app.data.normalizeClassName(classNameSelect.value);
            const classNames = app.data.getKnownStudentClassNames(classLevelSelect.value);
            classNameSelect.innerHTML = [
                '<option value="">Không khai báo lớp cụ thể</option>',
                ...classNames.map(className => `<option value="${app.data.sanitizeHTML(className)}">Lớp ${app.data.sanitizeHTML(className)}</option>`)
            ].join('');
        ...
        async addPlayerSubmit(editUsername) {
            ...
            const cl = app.data.normalizeClassLevel(document.getElementById('add-class').value);
            const className = app.data.normalizeClassName(document.getElementById('add-class-name')?.value || '');
            ...
            if (!['1', '2', '3', '4', '5'].includes(cl)) return alert('Cấp lớp không hợp lệ.');
            if (!['', 'male', 'female'].includes(gender || '')) return alert('Giới tính không hợp lệ.');
            if (className && !app.data.getKnownStudentClassNames(cl).includes(className)) return alert('Hãy chọn lớp cụ thể từ danh sách có sẵn.');
                    ...
                    if (window.supabase && user.id) {
                        try {
                            ...
                            const messages = {
                                ...
                                class_lookup_failed: 'Chưa tải được danh sách lớp. Vui lòng thử lại sau.',
    ...
    treasure: {
        ...
        renderStudentProfile(box) {
            const students = app.data.sortUsersByVietnameseName(app.data.users.filter(user => user.role?.toLowerCase() !== 'admin'));
            const options = students.map(user => `<option value="${app.data.sanitizeHTML(user.username)}">${app.data.sanitizeHTML(`${user.fullname} — ${app.data.getStudentClassLabel(user)} (${user.username})`)}</option>`).join('');
            box.innerHTML = `
    ...
    quest: {
        ...
        matchesAssignment(quest, user) {
            ...
            if (quest.assign_type === 'class') {
                const className = app.data.normalizeClassName(user.class_name);
                const target = app.data.normalizeClassName(quest.assign_target);
