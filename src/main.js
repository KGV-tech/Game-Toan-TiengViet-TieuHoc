...
const app = {
    ...
    admin: {
        ...
        renderTeamCompetitionBoard(box, id) {
            ...
            const startGrid = `<div class="team-stadium-start-grid" aria-hidden="true">${app.teamCompetition.STADIUM_LANES.map(lane => `<span class="team-stadium-start-grid__lane team-stadium-start-grid__lane--${lane.color}" data-stadium-lane="${lane.number}"></span>`).join('')}</div>`;
            const raceSurface = isLive
                ? `<div class="team-stadium-canvas" aria-label="Đường đua 8 lane">${startGrid}<div class="team-stadium-canvas__lanes">${cards}</div></div>`
