    ...
    admin: {
        ...
        renderTeamCompetitionForm(editId = null) {
            ...
            box.innerHTML = `<section class="team-competition-form team-competition-form--new" aria-label="Soạn trận thi đua nhóm">
                ...
                <section class="team-form-section"><div class="team-section-heading"><div><span class="team-section-kicker">Trình chiếu lớp</span><h4>Giao diện thi đua</h4><p>Xe/phi thuyền di chuyển theo điểm, không theo số câu đã nộp.</p></div></div><label class="team-field-label"><span>Chủ đề</span><select id="team-comp-presentation-theme" class="form-input"><option value="speed-race" ${draft.presentationTheme !== 'space-launch' ? 'selected' : ''}>Đường đua tốc độ — chạy ngang</option><option value="space-launch" ${draft.presentationTheme === 'space-launch' ? 'selected' : ''}>Phóng phi thuyền lên không gian — chạy dọc</option></select></label></section>
        ...
        renderTeamCompetitionBoard(box, id) {
            ...
            const presentationTheme = match.presentationTheme === 'space-launch' ? 'space-launch' : 'speed-race';
            const cards = match.teams.map((team, index) => {
                ...
                if (isLive) {
                    ...
                    const palette = ['ocean', 'sun', 'coral', 'violet', 'mint', 'rose', 'sky', 'amber'][index % 8];
                    const vehicle = presentationTheme === 'space-launch' ? '🚀' : ['🏎️', '🚙', '🚗', '🛻', '🏁', '🚕', '🚓', '🚜'][index % 8];
                    const ticks = Array.from({ length: 10 }, (_, tick) => `<span class="team-race-lane__track-tick" style="--tick:${tick}" aria-hidden="true"></span>`).join('');
                    return `<article class="team-race-lane team-race-lane--${presentationTheme} team-race-lane--${palette} team-race-lane--${teamStatusClass}" style="--race-progress:${Math.round(scoreProgress * 84)}%;--team-index:${index}" aria-label="${teamName}: ${score} trên 10, hạng tạm thời ${rank}"><div class="team-race-lane__identity"><span class="team-race-lane__rank">${rank}</span><div><span class="team-race-lane__kicker">Đội ${String(index + 1).padStart(2, '0')}</span><strong>${teamName}</strong><small>${teamStatus} · ${progress}/${total || '?'} câu</small></div><div class="team-race-lane__score"><strong>${score}</strong><span>/10 điểm</span></div></div><div class="team-race-lane__track" aria-hidden="true"><div class="team-race-lane__track-ticks">${ticks}</div><span class="team-race-lane__finish">🏁</span><span class="team-race-lane__vehicle">${vehicle}</span></div></article>`;
                }
            ...
            const leaderboard = isLive ? `<aside class="team-race-scoreboard" aria-label="Bảng xếp hạng tạm thời"><p>Bảng xếp hạng</p><ol>${rankedTeams.slice(0, 4).map(team => `<li><span>${app.teamCompetition.getTeamRank(match, team.id)}</span><strong>${app.data.sanitizeHTML(team.name)}</strong><b>${Number(team.score || 0).toLocaleString('vi-VN')}</b></li>`).join('')}</ol></aside>` : '';
            ...
            box.innerHTML = `<section class="team-competition-board ${isLive ? 'team-competition-board--live' : ''}" aria-label="Bảng thi đua nhóm"><div class="team-board-toolbar"><button type="button" class="btn-opt" onclick="app.admin.switchQuestMode('team')"><span aria-hidden="true">←</span><span>Danh sách trận</span></button><button type="button" class="btn-opt" onclick="app.admin.enterTeamBoardFullscreen()"><span aria-hidden="true">⛶</span><span>Mở toàn màn hình</span></button><span class="team-status-pill team-status-pill--${statusClass}">${status}</span></div><div class="team-race-stadium"><header class="team-board-hero"><div><p class="team-board-kicker">Đấu trường tri thức · Lớp ${app.data.sanitizeHTML(match.classlevel)}${match.className ? ` · ${app.data.sanitizeHTML(match.className)}` : ''}</p><h2>${app.data.sanitizeHTML(match.name || 'Trận thi đua')}</h2><p>${match.teams.length} đội cùng chinh phục vạch đích</p></div><div class="team-race-clock"><span>Thời gian còn lại</span><strong>${timerLabel}</strong></div>${leaderboard}<div class="team-board-summary" aria-label="Tóm tắt trận"><div><strong>${match.teams.length}</strong><span>đội</span></div><div><strong>${totalQuestions || '—'}</strong><span>câu/đề</span></div><div><strong>${match.teams.reduce((sum, team) => sum + team.memberUsernames.length, 0)}</strong><span>học sinh</span></div></div></header><div class="team-board-actions">${globalAction}</div><div class="team-board-grid">${cards}</div></div>${match.status === app.teamCompetition.STATUS.ENDED ? `<div class="team-board-ended-note">Trận đã kết thúc. Điểm nhóm được gán giống nhau cho từng thành viên trong bản ghi kết quả riêng.</div>` : ''}</section>`;
