    ...
    admin: {
        ...
        renderTeamCompetitionForm(editId = null) {
            ...
            box.innerHTML = `<section class="team-competition-form team-competition-form--new" aria-label="Soạn trận thi đua nhóm">
                ...
                <section class="team-form-section"><div class="team-section-heading"><div><span class="team-section-kicker">Trình chiếu lớp</span><h4>Giao diện thi đua</h4><p>Bộ asset được chọn sẽ hiển thị xe theo điểm, không theo số câu đã nộp.</p></div></div><label class="team-field-label"><span>Bộ asset trình chiếu</span><select id="team-comp-presentation-theme" class="form-input"><option value="stadium-3d" ${!['speed-race', 'space-launch'].includes(draft.presentationTheme) ? 'selected' : ''}>Sân vận động 3D — 8 lane màu</option><option value="speed-race" ${draft.presentationTheme === 'speed-race' ? 'selected' : ''}>Đường đua cổ điển — chạy ngang</option><option value="space-launch" ${draft.presentationTheme === 'space-launch' ? 'selected' : ''}>Phóng phi thuyền lên không gian — chạy dọc</option></select></label></section>
        ...
        renderTeamCompetitionBoard(box, id) {
            ...
            const presentationTheme = ['stadium-3d', 'speed-race', 'space-launch'].includes(match.presentationTheme) ? match.presentationTheme : 'stadium-3d';
            const stadiumLanes = presentationTheme === 'stadium-3d'
                ? app.teamCompetition.getStadiumLaneAssignments(match.teams.length)
                : [];
            const cards = match.teams.map((team, index) => {
                ...
                if (isLive) {
                    ...
                    if (presentationTheme === 'stadium-3d') {
            ...
            const titleCard = presentationTheme === 'stadium-3d'
                ? `<div class="team-stadium-title-card"><h2>${app.data.sanitizeHTML(match.name || 'Trận thi đua')}</h2></div>`
                : `<div class="team-stadium-title-card"><p class="team-board-kicker">Đấu trường tri thức · Lớp ${app.data.sanitizeHTML(match.classlevel)}${match.className ? ` · ${app.data.sanitizeHTML(match.className)}` : ''}</p><h2>${app.data.sanitizeHTML(match.name || 'Trận thi đua')}</h2><p>${match.teams.length} đội cùng tăng tốc theo điểm số</p></div>`;
            const raceSurface = isLive && presentationTheme === 'stadium-3d'
