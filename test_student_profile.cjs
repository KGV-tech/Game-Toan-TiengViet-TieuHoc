...
assert(source.includes("from('user_pets').select(SUPABASE_LIST_PROJECTIONS.user_pets).eq('user_username', username)"), 'Profile must load student pets with an explicit projection.');
assert(source.includes("from('user_quests').select(SUPABASE_LIST_PROJECTIONS.user_quests).eq('user_username', username)"), 'Profile must load student quest progress with an explicit projection.');
