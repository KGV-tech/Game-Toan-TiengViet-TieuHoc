...
assert.equal(team.PRESENTATION_THEME_OPTIONS.length, 5);
assert.deepEqual(team.PRESENTATION_THEME_OPTIONS.filter(item => item.available).map(item => item.id), ['speed-race', 'balloon-festival', 'treasure-island', 'space-launch', 'knowledge-garden']);
...
assert.equal(team.normalizeCompetition({ ...validConfig, presentationTheme: 'knowledge-garden' }).presentationTheme, 'knowledge-garden');
