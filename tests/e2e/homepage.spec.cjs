...
test('tablet ngang: học sinh có thể mở và đóng cửa sổ đổi mật khẩu trước khi đăng nhập', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.auth-password-intro')).toHaveText('Nhập đúng mật khẩu cũ để đặt mật khẩu mớicho tài khoản của bạn.');
  await expect(page.locator('.auth-password-intro br')).toHaveCount(1);
...
test('đăng ký dùng khung ngang hai cột và lưu lớp con, giới tính', async ({ page }, testInfo) => {
  ...
  expect(await page.locator('.register-panel').evaluate(panel => panel.getBoundingClientRect().width)).toBeLessThanOrEqual(735);
