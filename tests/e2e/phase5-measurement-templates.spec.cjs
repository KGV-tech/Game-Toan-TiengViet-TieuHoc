...
test('Phase 5 Bài 20 hiển thị phạm vi thực hành, Preview và gameplay trên laptop', async ({ page }) => {
  ...
  expect(gameplay).toMatchObject({ answers: 4, partAnswerCounts: [1, 1, 1, 1] });
  expect(gameplay.kinds).toHaveLength(4);
  expect(new Set(gameplay.kinds).size).toBe(1);
  expect(['mass', 'area', 'time', 'century']).toContain(gameplay.kinds[0]);
